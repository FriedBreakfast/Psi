...
namespace Psi {
  namespace Tvm {
    PSI_TEST_SUITE_FIXTURE(DerivedTest, Test::ContextFixture)

    PSI_TEST_CASE(GlobalConstArray) {
      const char *src =
      ...
      PSI_TEST_CHECK_EQUAL_RANGE(expected, expected+5, ptr, ptr+5);
    }

    PSI_TEST_CASE(FunctionReturnByteArray) {
      const char *src =
      ...
      PSI_TEST_CHECK_EQUAL_RANGE(x,x+8,r.r,r.r+8);
    }
    ...
    PSI_TEST_CASE(GlobalConstStruct) {
      const char *src =
      ...
      PSI_TEST_CHECK_EQUAL(expected, *ptr);
    }

    PSI_TEST_CASE(FunctionReturnStruct) {
      const char *src =
      ...
      PSI_TEST_CHECK_EQUAL(expected, result);
    }
    ...
    PSI_TEST_CASE(GlobalConstUnion) {
      const char *src =
      ...
      PSI_TEST_CHECK_EQUAL(ptr[0].a, 43256);
      PSI_TEST_CHECK_EQUAL(ptr[1].b[0], 14361);
      PSI_TEST_CHECK_EQUAL(ptr[1].b[1], 15);
    }

    PSI_TEST_CASE(FunctionReturnUnion) {
      const char *src =
        "%u = define (union i64 (array i32 #up2));\n"
      ...
      PSI_TEST_CHECK_EQUAL(r.u[1].b[0], b);
      PSI_TEST_CHECK_EQUAL(r.u[1].b[1], b);
    }

    PSI_TEST_CASE(FunctionParameterUnion) {
      const char *src =
        "%u = define (union i64 (array i32 #up2));\n"
      ...
      PSI_TEST_CHECK_EQUAL(r1.a, u1.a);
      PSI_TEST_CHECK_EQUAL(r2.b[0], u2.b[0]);
      PSI_TEST_CHECK_EQUAL(r2.b[1], u2.b[1]);
    }
