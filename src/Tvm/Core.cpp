...
namespace Psi {
  namespace Tvm {
    ...
    Global::Global() : ConstantValue(term_global) {
    }

    LLVMConstantBuilder::Constant Global::llvm_value_constant(LLVMConstantBuilder&, Term*) const {
      throw std::logic_error("Global variables called by normal LLVM build path");
    }
